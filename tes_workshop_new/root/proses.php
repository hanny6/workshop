...
include '../lib/db/dbconfig.php';
	if (isset($_POST['add_admXia1Zasww12Q'])) {
		...
		$sql_user = "INSERT INTO user (id_user,
			...
			?,
			?)";
		$sql_detail = "INSERT INTO detail_adm (id_user,
			name_user) VALUES(?,
			?)";
		$cek =$conn->query("SELECT*FROM user WHERE email_user='$email'")->num_rows;
		if ($email==="" || $pwd==="" || $nama ==="") {
			header("location:../root/add_adm.php?st=4");
		} else {
			if ($cek==0) {
				if ($statement = $conn->prepare($sql_user) AND $statement1 = $conn -> prepare($sql_detail)) {
						...
						if ($statement->execute() && $statement1->execute()) {
							header("location:../root/add_adm.php?st=1");
						} else {
							header("location:../root/add_adm.php?st=2");
						}
				}
			} else {
				header("location:../root/add_adm.php?st=3");
			}
		}
		$conn->close();
	} else {
