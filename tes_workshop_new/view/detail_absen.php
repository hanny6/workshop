<?php
	if (isset($_SESSION['sw'])) {
			          ...
			          while ($get_absen=$query_absen->fetch_assoc()) {
			            ...
			            $time_in = "$get_absen[jam_msk]";
			             if ($get_absen['jam_klr']==="") {
			             	$time_out = "<strong>Belum Presensi</strong>";
			             } else {
		...
		// 	$query = $conn->query("SELECT*FROM detail_user WHERE id_user='$id_anak'");
		// 	$get_user=$query->fetch_assoc();
		// 	$name = $get_user['name_user'];
		// 	$id_user = $get_user['id_user'];
		// 	if ($conn->query("SELECT*FROM data_absen WHERE id_user='$id_user'")->num_rows!==0) {
		...
		// 			// Tombol Export CSV
		// 			echo "<a class='btn btn-primary' href='export_csv.php?id_user=$id_user&id_month=$id_month&name=$name&month=$month&year=$year'>Export CSV</a>";
		// 		}
		// 	}
